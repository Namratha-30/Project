...
function getOrderItemByOrderIdUrl(){
	var baseUrl = $("meta[name=baseUrl]").attr("content");
...
function addOrderItem(event){
	...
	var url = getOrderItemByOrderIdUrl() + "/" + order_id+ "/" +"orderItems";
	var check = validateOrderItem(json);
	console.log(check);
	if(check) {
	console.log(check);
...
function createOrderItems(data,id,myList) {
	...
	for(var i in data){
		var e = data[i];
		var buttonHtml = '<button class="btn btn-sm btn-primary btnSend" id="editOrder'+e.id+'" onclick="displayEditOrderItem(' + e.id + ')">Edit</button>';
		var row = '<tr>'
		+ '<td>' + e.barcode + '</td>'
		+ '<td>'  + e.quantity + '</td>'
		+ '<td>' + e.sp + '</td>'
	...
	let currOrder = myList.find((e)=>e.id==id);
	
	if(currOrder.isInvoiceGenerated){
		for(var i in data){
			$('#editOrder'+data[i].id+'').attr('disabled',true);
