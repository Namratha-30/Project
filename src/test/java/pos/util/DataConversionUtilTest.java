...
public class DataConversionUtilTest extends AbstractUnitTest {
	...
	/*
	 * @Test public void testConvertProductPojoToData() throws ApiException {
	 * ProductPojo productpojo=new ProductPojo(); productpojo.setBarcode("126");
	 * productpojo.setMrp(27.3); productpojo.setName("hdvf"); productpojo.setId(1);
	 * 
	 * BrandPojo brandpojo=new BrandPojo(); brandpojo.setBrand("sjbfi");
	...
	public void testOrderItemstoInvoice() throws ApiException {
		...
		OrderInvoiceXmlList orderInvoiceXmlList = DataConversionUtil.convertToInvoiceDataList(orderItemPojoList,
				productPojoList);
		assertEquals(orderItemPojoList.size(), orderInvoiceXmlList.getOrderInvoiceData().size());
		assertEquals(orderInvoiceXmlList.getOrderInvoiceData().get(0).getName(),
				productService.get(orderItemPojoList.get(0).getProductId()).getName());
		assertEquals(orderInvoiceXmlList.getOrderInvoiceData().get(0).getQuantity(),
				orderItemPojoList.get(0).getQuantity());
		assertEquals(orderInvoiceXmlList.getOrderInvoiceData().get(0).getMrp(), orderItemPojoList.get(0).getSp(),
				0.001);
	}
	...
	public void testOrderItemsFormtoPojo() throws ApiException {
		...
		List<OrderItemPojo> orderItemPojoList = DataConversionUtil.convertOrderItemForms(barcode_product,
				orderItemForms);
		assertEquals(1, orderItemPojoList.size());
		assertEquals(orderItemForms[0].getBarcode(),
				productService.get(orderItemPojoList.get(0).getProductId()).getBarcode());
